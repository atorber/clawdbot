  ...
  return html`
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${pending.length > 0
          ? html`
              <div class="muted" style="margin-bottom: 8px;">${t("nodes.pendingSection")}</div>
              ${pending.map((req) => renderPendingDevice(req, props))}
            `
          : nothing}
        ${paired.length > 0
          ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${t("nodes.pairedSection")}</div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const age = typeof req.ts === "number" ? formatAgo(req.ts) : t("common.n/a");
  const role = req.role?.trim()
    ? `${t("nodes.roleLabel")}: ${req.role}`
    : `${t("nodes.roleLabel")}: -`;
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        ${tokens.length === 0
          ? html`<div class="muted" style="margin-top: 6px;">${t("nodes.tokens")}: ${t("nodes.tokensNone")}</div>`
          : html`
...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; labelKey: string }> = [
  { value: "deny", labelKey: "nodes.securityDeny" },
  { value: "allowlist", labelKey: "nodes.securityAllowlist" },
  { value: "full", labelKey: "nodes.securityFull" },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; labelKey: string }> = [
  { value: "off", labelKey: "nodes.askOff" },
  { value: "on-miss", labelKey: "nodes.askOnMiss" },
  { value: "always", labelKey: "nodes.askAlways" },
];
    ...
    <section class="card">
      ...
      ${state.formMode === "raw"
        ? html`<div class="callout warn" style="margin-top: 12px;">
            ${t("nodes.switchConfigForm")}
          </div>`
        : nothing}

      ${!state.ready
        ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodes.loadConfigToEditBindings")}</div>
            ...
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-meta">
                  ...
                  ${!supportsBinding
                    ? html`<div class="muted">${t("nodes.noNodesWithRun")}</div>`
                    : nothing}
                </div>
              </div>

              ${state.agents.length === 0
                ? html`<div class="muted">${t("nodes.noAgentsFound")}</div>`
    ...
    <div class="list" style="margin-top: 16px;">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodes.useDefaultLabel")} (${defaults.security})
                  </option>`
                : nothing}
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("nodes.askSubDefault") : `${t("common.default")}: ${defaults.ask}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t("nodes.useDefaultLabel")} (${defaults.ask})
                  </option>`
                : nothing}
              ${ASK_OPTIONS.map(
                (option) =>
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t("nodes.useDefaultLabel")} (${defaults.askFallback})
                  </option>`
                : nothing}
              ${SECURITY_OPTIONS.map(
                (option) =>
        ...
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults
              ...
              : autoIsDefault
                ? `${t("nodes.useDefaultLabel")} (${defaults.autoAllowSkills ? "on" : "off"}).`
                : `${t("nodes.override")} (${autoEffective ? "on" : "off"}).`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
